...
fn make_post_lookup(agg_function: PostLookupAggregateFunction, has_group_by: bool) -> PostLookup {
    PostLookup {
        order_by: None,
        limit: None,
        returned_cols: None,
...
fn bench_string_agg(c: &mut Criterion) {
    ...
    for &num_keys in &[10, 50, 100] {
        for &values_per_key in &[5, 20, 50] {
            ...
            let post_lookup =
                make_post_lookup(make_string_agg_op(",", DistinctOption::NotDistinct), false);
...
fn bench_array_agg(c: &mut Criterion) {
    ...
    for &num_keys in &[10, 50, 100] {
        for &values_per_key in &[5, 20, 50] {
            ...
            let post_lookup = make_post_lookup(make_array_agg_op(), false);
...
fn bench_group_concat(c: &mut Criterion) {
    ...
    let post_lookup = make_post_lookup(make_group_concat_op(","), false);
...
fn bench_string_agg_with_group_by(c: &mut Criterion) {
    ...
    let data = make_string_agg_data(50, 20, ",");
    let post_lookup = make_post_lookup(make_string_agg_op(",", DistinctOption::NotDistinct), true);
...
fn bench_string_agg_distinct(c: &mut Criterion) {
    ...
    for &num_keys in &[10, 50] {
        ...
        let post_lookup =
            make_post_lookup(make_string_agg_op(",", DistinctOption::IsDistinct), false);
