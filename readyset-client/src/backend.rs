...
//!
//! In a distributed deployment, a component (such as a readyset-server instance) may go down, causing
...
//! be enabled for, before allowing affected queries to be retried against ReadySet.
//!
...
enum PrepareMeta {
    ...
    FailedToParse,
    /// Query could not be rewritten for processing in ReadySet
    FailedToRewrite,
...
/// execution of a prepared statement on either ReadySet or the upstream
/// connection.
struct CachedPreparedStatement<DB>
where
    DB: UpstreamDatabase,
{
    /// Indicates if the statment was prepared for ReadySet, Fallback, or Both
    ...
    DB: UpstreamDatabase,
{
    ...
    // all queries previously prepared on ReadySet or upstream, mapped by their ID.
    ...
    /// checked by ReadySet view nodes.
    ticket: Option<Timestamp>,
    ...
    Handler: 'static + QueryHandler,
{
    ...
    /// Executes query on the upstream database, for when it cannot be parsed or executed by ReadySet.
    ...
    async fn mirror_prepare(
        ...
        // Update ReadySet migration state for query
        match &noria_res {
            ...
            Some(Err(e)) => {
                if e.caused_by_view_not_found() {
                    ...
                    error!(
                        error = %e,
                        "Error received from ReadySet during mirror_prepare()"
                    );
                }
    ...
    fn plan_prepare_select(&mut self, stmt: nom_sql::SelectStatement) -> PrepareMeta {
        ...
        if rewrite::process_query(&mut rewritten).is_err() {
            ...
            // For select statements we will always try to check with ReadySet if it already migrated,
    ...
    /// Prepares a query on ReadySet and upstream based on the provided PrepareMeta
    async fn do_prepare(
        &mut self,
        meta: &PrepareMeta,
        query: &str,
            ...
            Err(noria_err) => {
                if let Some(info) = ex_info {
                    if noria_err.is_networking_related() {
                        ...
                        // unsupported. These queries will likely fail on each query to ReadySet,
                        // introducing increased latency.
                        info.execute_unsupported();
                    }
                }
                if !matches!(noria_err, ReadySetError::ReaderMissingKey) {
                    warn!(error = %noria_err, "Error received from ReadySet, sending query to fallback");
                }
    ...
    /// Attempts to migrate a query on ReadySet, after it was marked as Succesful in the cache. If the
    /// migration is succesful, the cached entry is marked as such and will attempt to resolve
    /// ReadySet first in the future
    ///
    /// # Panics
    ///
    ...
    async fn update_noria_prepare(
        ...
        // At this point we got a succesful ReadySet prepare, so we want to replace the Upstream result
        // with a Both result
    ...
    fn invalidate_prepared_cache_entry(prep: &mut PrepareResult<DB>) {
        match prep {
            //We can't invalidate the ReadySet entry if there is no fallback
    ...
    pub async fn execute(
        ...
        if let Some(e) = event.noria_error.as_ref() {
            if e.caused_by_view_not_found() {
                // This can happen during cascade execution if the ReadySet query was removed from
                // another connection
    ...
    /// Forwards a `DROP CACHE` request to ReadySet
    ...
    async fn query_noria_extensions<'a>(
        ...
        event.sql_type = SqlQueryType::Other; // Those will get cleared if it was not destined to ReadySet
        event.destination = Some(QueryDestination::Readyset);
        ...
        let result = match parse_result {
            ...
            Err(e) => {
                if !matches!(e, ReadySetError::ReaderMissingKey) {
                    warn!(error = %e, "Error received from ReadySet, sending query to fallback");
                }
    ...
    async fn query_adhoc_non_select<'a>(
        ...
        let res = {
            // Upstream reads are tried when ReadySet reads produce an error. Upstream writes are done
            ...
            if let Some(ref mut upstream) = self.upstream {
                ...
                let res = match parsed_query {
                    ...
                    // messages are dropped - we do not support transactions in ReadySet standalone.
