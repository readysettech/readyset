...
impl ReaderHandle {
    ...
    /// Build a [`ViewQuery`] for performing a lookup against this [`ReaderHandle`] given keys and
    /// binops used for post-read filters
    ...
    fn build_view_query(
        ...
        let keys = if raw_keys.is_empty() {
            ...
            let remap_key =
                |key: &[DfValue], idx: &usize, key_type: &DfType| -> ReadySetResult<_> {
                    Ok(match key_remap {
                        Some(remap) => {
