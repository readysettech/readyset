...
use rand::thread_rng;
use readyset_data::{range, DfType, DfValue, IntoBoundedRange};
use readyset_errors::{
...
impl KeyComparison {
    ...
    pub fn from_key_and_operator(
        ...
        let inner = match operator {
            Greater => key.greater_than(),
            GreaterOrEqual => key.greater_than_or_equal_to(),
            Less => key.less_than(),
            LessOrEqual => key.less_than_or_equal_to(),
...
impl From<BoundedRange<Vec1<DfValue>>> for KeyComparison {
    /// Converts to a [`KeyComparison::Range`]
...
impl Arbitrary for KeyComparison {
    ...
    fn arbitrary_with(_: Self::Parameters) -> Self::Strategy {
        ...
        prop_oneof![
            ...
            (bound(), bound()).prop_map(|(l, u)| KeyComparison::Range((l, u)))
        ]
        .boxed()
    }
    ...
    mod build_view_query {
        ...
        fn compound_range_open() {
            ...
            assert_eq!(
                query.key_comparisons,
                vec![KeyComparison::Range(
                    vec1![DfValue::from(1), DfValue::from("a")].greater_than()
                )]
            );
        }
