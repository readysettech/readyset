...
pub fn thread_memory_stats() -> Result<String, Error> {
    // Stats are cached. Need to advance epoch to refresh.
    epoch::advance()?;
    thread_memory_stats_inner()
}

fn thread_memory_stats_inner() -> Result<String, Error> {
    let mut memory_stats = "Memory stats by thread:\n".to_string();
...
pub fn memory_and_per_thread_stats() -> Result<String, Error> {
    // Stats are cached. Need to advance epoch to refresh.
    epoch::advance()?;
    let mut memory_stats = format!("Memory stats summary: {:?}\n", fetch_stats_inner()?);
    memory_stats.push_str(thread_memory_stats_inner()?.as_str());
