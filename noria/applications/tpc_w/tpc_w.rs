...
async fn make(
    ...
    g.extend_recipe(recipe.clone().parse().unwrap())
        .await
        .unwrap();
...
impl Backend {
    async fn extend(&mut self, query: &str) {
        ...
        self.g
            .extend_recipe(new_recipe.clone().parse().unwrap())
            .await
            .unwrap();
...
async fn main() {
    ...
    let matches = App::new("tpc_w")
        ...
        .arg(
            Arg::new("parallel_prepopulation")
        ...
        .arg(
            Arg::new("single_query_migration")
    ...
    let read_scale: f32 = matches.value_of_t_or_exit("read");
    let write_to = matches.value_of("write_to").unwrap();
    let write: f32 = matches.value_of_t_or_exit("write");
