...
impl Node {
    ...
    pub fn parent_columns(
        ...
        match self.column_source(&[column])? {
            ColumnSource::ExactCopy(ColumnRef { node, columns }) => {
                Ok(vec![(node, columns.into_iter().next())])
            }
            ColumnSource::GeneratedFromColumns(refs) => {
                Ok(refs.into_iter().map(|x| (x.node, None)).collect())
            }
            ColumnSource::RequiresFullReplay(nodes) => {
                Ok(nodes.into_iter().map(|x| (x, None)).collect())
            }
            ColumnSource::Union(refs) => Ok(refs
                .into_iter()
    ...
    pub fn resolve(&self, i: usize) -> Result<Option<Vec<(NodeIndex, usize)>>, ReadySetError> {
        match self.column_source(&[i])? {
            ColumnSource::ExactCopy(ColumnRef { node, columns }) => {
                Ok(Some(vec![(node, columns[0])]))
            }
            ColumnSource::Union(refs) => Ok(Some(
                refs.into_iter()
