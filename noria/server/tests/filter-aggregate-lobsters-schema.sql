...
CREATE VIEW all_comment_votes AS SELECT votes.comment_id, votes.user_id, votes.vote FROM votes WHERE votes.story_id IS NULL;

CREATE VIEW FULL_comment_upvotes AS SELECT all_comment_votes.comment_id AS id, COUNT(CASE WHEN all_comment_votes.vote = 1 THEN all_comment_votes.vote END) as votes FROM all_comment_votes GROUP BY all_comment_votes.comment_id;

CREATE VIEW FULL_comment_downvotes AS SELECT all_comment_votes.comment_id AS id, COUNT(CASE WHEN all_comment_votes.vote = 0 THEN all_comment_votes.vote END) as votes FROM all_comment_votes GROUP BY all_comment_votes.comment_id;
...
CREATE VIEW all_story_votes AS SELECT votes.story_id, votes.user_id, votes.vote FROM votes WHERE votes.comment_id IS NULL;

CREATE VIEW FULL_story_upvotes AS SELECT all_story_votes.story_id AS id, COUNT(CASE WHEN all_story_votes.vote = 1 THEN all_story_votes.vote END) as votes FROM all_story_votes GROUP BY all_story_votes.story_id;

CREATE VIEW FULL_story_downvotes AS SELECT all_story_votes.story_id AS id, COUNT(CASE WHEN all_story_votes.vote = 0 THEN all_story_votes.vote END) as votes FROM all_story_votes GROUP BY all_story_votes.story_id;
