...
/// Routes requests from an HTTP server to ReadySet server workers and controllers.
...
impl NoriaServerHttpRouter {
    ...
    /// Routes requests for a ReadySet server http router received on `http_listener`
            ...
            (&Method::POST, "/metrics_dump") => {
                ...
                let res = match render {
                    ...
                    None => res
                        ...
                        .body(hyper::Body::from("ReadySet metrics were not enabled. To fix this, run ReadySet with --noria-metrics".to_string())),
                };
                Box::pin(async move { Ok(res.unwrap()) })
            }
