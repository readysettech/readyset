...
async fn main() {
    ...
    benchmark_application.install_schemas_and_queries().await.unwrap();
    benchmark_application.load_data(nemployees).await.unwrap();
    benchmark_application.run_benchmarks(ntests, nemployees).await.unwrap();
}
...
impl BenchmarkApplication {
    ...
    pub async fn run_benchmarks(&mut self, ntests : i32, nemployees : i32) -> result::Result<(), String> {
        ...
        for i in &employee_ids_to_query {
            ...
            let dept_manager_string = format!("SELECT * FROM dept_manager WHERE emp_no={}", i.to_string());
            ...
            mysql_employee_count += self.mysql
                .query_map(
                    employee_string,
                    |(emp_no, age, gender)| {
                        Employee { emp_no, age, gender }
                    },
            ).unwrap().len();
            mysql_manager_count += self.mysql
                .query_map(
                    dept_manager_string,
                    |(emp_no, dept_no, id)| {
                        DeptManager { emp_no, dept_no, id }
                    },
            ).unwrap().len();
            mysql_dept_emp_count += self.mysql
                .query_map(
                    dept_emp_string,
                    |(emp_no, dept_no, id)| {
                        DeptEmp { emp_no, dept_no, id }
                    },
            ).unwrap().len();
            mysql_salary_count += self.mysql
                .query_map(
                    salary_string,
                    |(emp_no, salary, id)| {
                        Salary { emp_no, salary, id }
                    },
            ).unwrap().len();
        }
        ...
        for i in employee_ids_to_query.clone() {
            noria_employee_count += emp_view.lookup(&[i.into()], true).await.unwrap().iter().len();
            noria_dept_emp_count += dept_emp_view.lookup(&[i.into()], true).await.unwrap().iter().len();
            noria_manager_count += manager_view.lookup(&[i.into()], true).await.unwrap().iter().len();
            noria_salary_count += salary_view.lookup(&[i.into()], true).await.unwrap().iter().len();
        }
        ...
        let noria_lookup : Vec<Vec<DataType>> = employee_ids_to_query.iter().map(|e_id| vec!(e_id.clone().into())).collect();
        emp_view.multi_lookup(noria_lookup.clone(), true).await.unwrap();
        dept_emp_view.multi_lookup(noria_lookup.clone(), true).await.unwrap();
        manager_view.multi_lookup(noria_lookup.clone(), true).await.unwrap();
        salary_view.multi_lookup(noria_lookup.clone(), true).await.unwrap();
        ...
        assert!(mysql_employee_count == noria_employee_count, "Found incorrect number of employees");
        assert!(mysql_dept_emp_count == noria_dept_emp_count, "Found incorrect number of dept_emp");
        assert!(mysql_manager_count == noria_manager_count, "Found incorrect number of managers");
        assert!(mysql_salary_count == noria_salary_count, "Found incorrect number of salaries\n");
    ...
    pub async fn install_schemas_and_queries(&mut self) -> result::Result<(), String> {
        ...
        self.mysql.query_drop(r"DROP TABLE employees;
        DROP TABLE dept_manager;
        DROP TABLE dept_emp;
        DROP TABLE salaries;");
 
        println!("Initializing all mysql tables.");
        self.mysql.query_drop(r"
            CREATE TABLE employees (
                `emp_no`      INT             NOT NULL,
                `age`         INT             NOT NULL,
                `gender`      VARCHAR(1)  NOT NULL,    
    ...
    pub async fn load_data(&mut self, n_employees : i32) -> result::Result<(), String> {
        ...
        for i in 1..n_employees {
            ...
            employees_mysql.push(Employee {emp_no : i, age : age, gender : gender.to_string()});
        }

        let mut employees_mutator = self.g.table("employees").await.unwrap();
        employees_mutator.i_promise_dst_is_same_process();
        employees_mutator.perform_all(employees.clone()).await.unwrap();
        self.mysql.exec_batch(
            r"INSERT INTO employees (emp_no, age, gender)
              VALUES (:emp_no, :age, :gender)",
            employees_mysql.iter().map(|d| params! {
                "emp_no" => d.emp_no,
                "age" => d.age,
        ...
        for i in 1..n_employees {
            ...
            dep_employees_mysql.push(DeptEmp {emp_no : i, dept_no : department, id : i});
        }
        let mut dep_emp_mutator = self.g.table("dept_emp").await.unwrap();
        dep_emp_mutator.i_promise_dst_is_same_process();
        dep_emp_mutator.perform_all(dep_employees.clone()).await.unwrap();
        self.mysql.exec_batch(
            r"INSERT INTO dept_emp (emp_no, dept_no, id)
              VALUES (:emp_no, :dept_no, :id)",
            dep_employees_mysql.iter().map(|d| params! {
                "emp_no" => d.emp_no,
                "dept_no" => d.dept_no,
                "id" => d.id
            })
        ...
        for i in 1..n_employees*3 {
            ...
            salaries_mysql.push(Salary {emp_no : emp, salary : salary, id : i});
        }
        ...
        self.mysql.exec_batch(
            ...
            salaries_mysql.iter().map(|d| params! {
                "emp_no" => d.emp_no,
                "salary" => d.salary,
                "id" => d.id
            })
