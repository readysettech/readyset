    ...
    let (coltype, character_set, decimals, column_length) = match col.column_type {
        DfType::TinyInt => {
            let sql_type = col
                .base
                .clone()
                .unwrap_or_else(|| default_column_base(col, SqlType::TinyInt(None)))
                .sql_type;
        ...
        DfType::Text(_) => {
            let sql_type = col
                .base
                .clone()
                .unwrap_or_else(|| default_column_base(col, SqlType::Text))
                .sql_type;
            let charset = mysql_common::collations::Collation::from(
                mysql_common::collations::CollationId::UTF8MB4_GENERAL_CI,
            );
        ...
        DfType::Blob => {
            ...
            colflags |= mysql_srv::ColumnFlags::BLOB_FLAG;
            colflags |= mysql_srv::ColumnFlags::BINARY_FLAG;
            (MYSQL_TYPE_BLOB, DEFAULT_CHARACTER_SET_NUMERIC, 0, lenght)
        }
        ...
        DfType::Binary(_) => {
            colflags |= mysql_srv::ColumnFlags::BINARY_FLAG;
            (MYSQL_TYPE_STRING, DEFAULT_CHARACTER_SET_NUMERIC, 0, 1024)
        }
        DfType::VarBinary(_) => {
            colflags |= mysql_srv::ColumnFlags::BINARY_FLAG;
            (
