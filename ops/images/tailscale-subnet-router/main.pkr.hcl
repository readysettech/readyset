...
locals {
  ...
  service               = "readyset-tailscale-subnet-router"
  ssh_username          = "ubuntu"
...
variable "ami_description" {
  ...
  default     = "Server image to be used in a ReadySet cluster"
}
...
build {
  ...
  provisioner "ansible" {
    playbook_file    = "${path.root}/../provisioners/ansible/playbooks/tailscale-subnet-router.yml"
    ansible_env_vars = ["ANSIBLE_CONFIG=${path.root}/../provisioners/ansible/ansible.cfg"]
    user             = "ec2-user"
  }
}
