...
variable "ami_description" {
  type    = string
  default = "debezium_connector image to be used in a Readyset cluster."
}
...
source "amazon-ebs" "debezium_connector" {
  ami_name                  = "debezium_connector-${local.date}-${var.short_commit_id}"
...
build {
  sources = ["source.amazon-ebs.debezium_connector"]

  provisioner "file" {
    source      = "binaries/target/release/debezium_connector"
    destination = "/tmp/debezium_connector"
  }

  provisioner "file" {
    source      = "files/debezium-connector.service"
    destination = "/tmp/debezium-connector.service"
  }
  ...
  provisioner "shell" {
    inline = [
      ...
      "sudo mv /tmp/debezium-connector.service /etc/systemd/system/debezium-connector.service",
      "sudo chown root:root /etc/systemd/system/debezium-connector.service",
      "sudo touch /etc/default/debezium-connector",
      "sudo systemctl daemon-reload",
      "sudo systemctl disable debezium-connector.service"
    ]
  }
}
