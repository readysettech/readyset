...
impl<B: MySqlShim<S> + Send, S: AsyncWrite + AsyncRead + Unpin + Send>
    MySqlIntermediary<B, S>
{
    ...
    async fn init(&mut self) -> Result<(bool, Option<String>), io::Error> {
        ...
        self.stream.enqueue_packet(init_packet);
        self.stream.flush().await?;

        let packet = self.stream.next().await?.ok_or_else(|| {
        ...
        self.stream.set_seq(packet.seq + 1);
            ...
            || password.is_empty()
        {
            ...
            auth_switch_request_packet.push(0);
            self.stream.enqueue_packet(auth_switch_request_packet);
            self.stream.flush().await?;
