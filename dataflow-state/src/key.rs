...
impl From<BoundedRange<Vec1<DfValue>>> for RangeKey {
    fn from(range: BoundedRange<Vec1<DfValue>>) -> Self {
        use Bound::*;
        let len = match (&range.0, &range.1) {
...
impl RangeKey {
    ...
    {
        <RangeKey as From<BoundedRange<_>>>::from((
