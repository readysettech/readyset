...
pub enum RangeKey {
    ...
    Unbounded,
    Single(BoundRange<DfValue>),
    Double(BoundRange<(DfValue, DfValue)>),
    Tri(BoundRange<(DfValue, DfValue, DfValue)>),
    Quad(BoundRange<(DfValue, DfValue, DfValue, DfValue)>),
    Quin(BoundRange<(DfValue, DfValue, DfValue, DfValue, DfValue)>),
    Sex(BoundRange<(DfValue, DfValue, DfValue, DfValue, DfValue, DfValue)>),
    Multi(BoundRange<Box<[DfValue]>>),
}
...
impl RangeKey {
    ...
    pub fn upper_bound(&self) -> Bound<Vec<&DfValue>> {
        match self {
            ...
            RangeKey::Single(BoundRange(_, upper)) => upper.as_ref().map(|dt| vec![dt]),
            RangeKey::Double(BoundRange(_, upper)) => {
                upper.as_ref().map(|dts| dts.elements().collect())
            }
            RangeKey::Tri(BoundRange(_, upper)) => {
                upper.as_ref().map(|dts| dts.elements().collect())
            }
            RangeKey::Quad(BoundRange(_, upper)) => {
                upper.as_ref().map(|dts| dts.elements().collect())
            }
            RangeKey::Quin(BoundRange(_, upper)) => {
                upper.as_ref().map(|dts| dts.elements().collect())
            }
            RangeKey::Sex(BoundRange(_, upper)) => {
                upper.as_ref().map(|dts| dts.elements().collect())
            }
            RangeKey::Multi(BoundRange(_, upper)) => upper.as_ref().map(|dts| dts.iter().collect()),
        }
    }
    ...
    pub fn len(&self) -> Option<usize> {
        match self {
            RangeKey::Unbounded
            | RangeKey::Multi(BoundRange(Bound::Unbounded, Bound::Unbounded)) => None,
            ...
            RangeKey::Multi(
                BoundRange(Bound::Included(k), _)
                | BoundRange(Bound::Excluded(k), _)
                | BoundRange(_, Bound::Included(k))
                | BoundRange(_, Bound::Excluded(k)),
            ) => Some(k.len()),
        }
    }
    ...
    pub(crate) fn into_point_keys(self) -> BoundRange<PointKey> {
        use Bound::*;

        macro_rules! point_keys {
            ($r:ident, $variant:ident) => {
                BoundRange($r.0.map(PointKey::$variant), $r.1.map(PointKey::$variant))
            };
        }

        match self {
            RangeKey::Unbounded => BoundRange(Unbounded, Unbounded),
            RangeKey::Single(BoundRange(l, u)) => {
                BoundRange(l.map(PointKey::Single), u.map(PointKey::Single))
            }
    ...
    pub fn as_bound_pair(&self) -> BoundRange<Vec<DfValue>> {
        fn as_bound_pair<T>(bound_pair: &BoundRange<T>) -> BoundRange<Vec<DfValue>>
        where
        ...
        match self {
            RangeKey::Unbounded => BoundRange(Bound::Unbounded, Bound::Unbounded),
            RangeKey::Single(BoundRange(lower, upper)) => BoundRange(
            ...
            RangeKey::Multi(BoundRange(lower, upper)) => BoundRange(
