            ...
            match option.option {
                ...
                sqlparser::ast::ColumnOption::OnUpdate(expr) => {
                    ...
                    if let sqlparser::ast::Expr::Function(Function { name, args, .. }) = expr
                        && name.to_string().eq_ignore_ascii_case("CURRENT_TIMESTAMP")
                    {
                        let arg = match args {
                ...
                sqlparser::ast::ColumnOption::Generated {
                    ...
                    let kind = match generated_as {
                        GeneratedAs::Always | GeneratedAs::ExpStored => GeneratedKind::Always,
                        GeneratedAs::ByDefault => GeneratedKind::ByDefault,
                    };
...
pub struct GeneratedColumn {
    /// The generation expression, or `None` for identity columns.
    pub expr: Option<Expr>,
    /// `ALWAYS` vs `BY DEFAULT` (for identity columns).
    pub kind: GeneratedKind,
    /// Whether the column is `STORED` (vs `VIRTUAL`). Only meaningful when `expr` is `Some`.
    pub stored: bool,
}

impl Arbitrary for GeneratedColumn {
    type Parameters = ();
    ...
    fn arbitrary_with(_: Self::Parameters) -> Self::Strategy {
        ...
        prop_oneof![
            // Computed column: GENERATED ALWAYS AS (expr) [STORED | VIRTUAL]
            (any::<Expr>(), any::<bool>()).prop_map(|(expr, stored)| GeneratedColumn {
                expr: Some(expr),
                kind: GeneratedKind::Always,
                stored,
            }),
            // Identity column: GENERATED ALWAYS AS IDENTITY
            Just(GeneratedColumn {
                expr: None,
                kind: GeneratedKind::Always,
                stored: false,
            }),
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match &self.expr {
            ...
            None => {
                ...
                write!(f, "GENERATED {kind} AS IDENTITY")
            }
        }
    }
}
...
mod tests {
    ...
    fn parse_columns(dialect: Dialect, sql: &str) -> Vec<ColumnSpecification> {
        ...
        let stmts = sqlparser::parser::Parser::parse_sql(sqlparser_dialect.as_ref(), sql)
            .expect("failed to parse SQL");
        let stmt = stmts.into_iter().next().expect("no statements parsed");
        match stmt {
            sqlparser::ast::Statement::CreateTable(ct) => ct
                .columns
                .into_iter()
                .map(|col| {
                    col.try_into_dialect(dialect)
                        .expect("column conversion failed")
                })
                .collect(),
    ...
    #[test]
    fn postgres_generated_always_as_identity() {
        ...
        let col = &cols[0];
        assert_eq!(col.column.name.as_str(), "id");
        let gc = col.generated.as_ref().expect("should have generated info");
        assert_eq!(gc.expr, None);
        assert_eq!(gc.kind, GeneratedKind::Always);
    }

    #[test]
    fn postgres_generated_by_default_as_identity() {
        let cols = parse_columns(
            Dialect::PostgreSQL,
        ...
        let gc = cols[0]
            .generated
            .as_ref()
            .expect("should have generated info");
        assert_eq!(gc.expr, None);
        assert_eq!(gc.kind, GeneratedKind::ByDefault);
    }

    #[test]
    fn postgres_generated_always_as_expr_stored() {
        let cols = parse_columns(
            Dialect::PostgreSQL,
        ...
        let gc = cols[2]
            ...
            .as_ref()
            .expect("should have generated info");
        assert!(gc.expr.is_some(), "computed column should have an expr");
        assert_eq!(gc.kind, GeneratedKind::Always);
        assert!(gc.stored);
    }

    #[test]
    fn mysql_generated_column() {
        let cols = parse_columns(
            Dialect::MySQL,
