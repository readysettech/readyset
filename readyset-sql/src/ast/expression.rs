...
impl TryFromDialect<sqlparser::ast::Expr> for Expr {
    fn try_from_dialect(
        ...
        match value {
            ...
            // `Is`/`IsNot` are null-safe equality, which is exactly what the `DISTINCT FROM`
            // family means. Routing through them also makes these predicates rather than
            // arithmetic, so they push down to the relation they reference.
