...
//! [`readyset_sql::analysis::visit_mut`].
...
pub trait Visitor<'ast>: Sized {
    ...
    fn visit_index_key_part(&mut self, key_part: &'ast IndexKeyPart) -> Result<(), Self::Error> {
...
pub fn walk_index_key_part<'ast, V: Visitor<'ast>>(
    visitor: &mut V,
    key_part: &'ast IndexKeyPart,
) -> Result<(), V::Error> {
    match key_part {
...
mod tests {
    ...
    fn node_count(query: SqlQuery) -> usize {
        let mut counter = NodeCounter::default();
        counter.visit_sql_query(&query).unwrap();
        counter.0
    }
