...
pub trait VisitorMut<'ast>: Sized {
    ...
    fn visit_create_database_statement(
        &mut self,
        create_database_statement: &'ast mut CreateDatabaseStatement,
    ) -> Result<(), Self::Error> {
    ...
    fn visit_drop_database_statement(
        &mut self,
        drop_database_statement: &'ast mut DropDatabaseStatement,
    ) -> Result<(), Self::Error> {
    ...
    fn visit_create_index_statement(
        &mut self,
        create_index_statement: &'ast mut CreateIndexStatement,
    ) -> Result<(), Self::Error> {
    ...
    fn visit_drop_rls_statement(
        &mut self,
        drop_rls_statement: &'ast mut DropRlsStatement,
    ) -> Result<(), Self::Error> {
    ...
    fn visit_create_rls_statement(
        &mut self,
        create_rls_statement: &'ast mut CreateRlsStatement,
    ) -> Result<(), Self::Error> {
    ...
    fn visit_rls_rule(
        &mut self,
        col: &'ast mut Column,
        var: &'ast mut Variable,
    ) -> Result<(), Self::Error> {
        walk_rls_rule(self, col, var)
    }

    fn visit_rls_if_not_exists(&mut self, _if_not_exists: &mut bool) -> Result<(), Self::Error> {
        Ok(())
    }
    ...
    fn visit_create_database_option(
        &mut self,
        _create_database_option: &'ast mut CreateDatabaseOption,
    ) -> Result<(), Self::Error> {
        Ok(())
    }
    ...
    fn visit_flush_cache_statement(
        &mut self,
        _flush_cache_statement: &'ast mut FlushCacheStatement,
    ) -> Result<(), Self::Error> {
        Ok(())
    }
...
pub fn walk_table_expr<'ast, V: VisitorMut<'ast>>(
    ...
    for col_alias in &mut table_expr.column_aliases {
...
pub fn walk_create_database_statement<'a, V: VisitorMut<'a>>(
    visitor: &mut V,
    create_database_statement: &'a mut CreateDatabaseStatement,
) -> Result<(), V::Error> {
    visitor.visit_sql_identifier(&mut create_database_statement.name)?;
    if let Ok(options) = &mut create_database_statement.options {
        for option in options {
...
pub fn walk_drop_database_statement<'a, V: VisitorMut<'a>>(
    visitor: &mut V,
    drop_database_statement: &'a mut DropDatabaseStatement,
) -> Result<(), V::Error> {
    for name in &mut drop_database_statement.names {
        visitor.visit_sql_identifier(name)?;
    }
    Ok(())
}

pub fn walk_drop_rls_statement<'a, V: VisitorMut<'a>>(
    visitor: &mut V,
    drop_rls_statement: &'a mut DropRlsStatement,
) -> Result<(), V::Error> {
    if let Some(table) = &mut drop_rls_statement.table {
        visitor.visit_table(table)?;
    }
    Ok(())
}

pub fn walk_create_rls_statement<'a, V: VisitorMut<'a>>(
    visitor: &mut V,
    create_rls_statement: &'a mut CreateRlsStatement,
) -> Result<(), V::Error> {
    visitor.visit_table(&mut create_rls_statement.table)?;
    for (col, var) in &mut create_rls_statement.policy {
        visitor.visit_rls_rule(col, var)?;
    }
    visitor.visit_rls_if_not_exists(&mut create_rls_statement.if_not_exists)?;
    Ok(())
}

pub fn walk_rls_rule<'a, V: VisitorMut<'a>>(
    visitor: &mut V,
    col: &'a mut Column,
    var: &'a mut Variable,
) -> Result<(), V::Error> {
...
pub fn walk_create_index_statement<'a, V: VisitorMut<'a>>(
    visitor: &mut V,
    create_index_statement: &'a mut CreateIndexStatement,
) -> Result<(), V::Error> {
    visitor.visit_table(&mut create_index_statement.table_name)?;

    for column in &mut create_index_statement.columns {
        visitor.visit_column(column)?;
    }

    if let Some(predicate) = &mut create_index_statement.predicate {
