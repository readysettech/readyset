...
# image name (i.e. "readyset-rust-ubuntu2204-builder-base") and optional tags, create a multi-arch
# manifest that references the architecture-specific images for that image name (i.e.
# "readyset-rust-ubuntu2204-builder-base:$VERSION-amd64"). If no tags are provided, the manifest
# is tagged with $VERSION. If tags are provided as arguments, the manifest is created and pushed
...
if [ $# -eq 0 ]; then
  ...
  for param in "$@"; do
    create_and_push_manifest "$param"
  done
fi
