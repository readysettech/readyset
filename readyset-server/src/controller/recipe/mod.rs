...
use self::registry::MatchedCache;
use super::sql;
...
impl Recipe {
    ...
    pub fn reused_caches(&self, name: &Relation) -> Option<&Vec1<MatchedCache>> {
        self.registry.reused_caches(name)
    }
        ...
        for change in changes {
            match change {
                ...
                Change::CreateCache(mut ccqs) => {
                    ...
                    let name = ccqs.name.clone().unwrap_or_else(|| {
                        format!("q_{:x}", {
                            let mut hasher = DefaultHasher::new();
                            statement.hash(&mut hasher);
                            schema_search_path.hash(&mut hasher);
                            hasher.finish()
                        })
                        .into()
                    });
                    ...
                    // The expression is not present, so attempt to migrate
                    if aliased {
                        // If we fail to migrate the query, see if we can use an existing cached
                        // query.
                        if let Err(error) =
                            self.inc
                                .add_query(Some(name.clone()), statement.clone(), mig)
                        {
                            let caches = self.registry.caches_for_query(statement.clone())?;
                            if caches.is_empty() {
                                return Err(error);
                            } else {
                                #[allow(clippy::unwrap_used)]
                                // we checked that chaches is not empty
                                self.registry.add_reused_caches(
                                    name.clone(),
                                    Vec1::try_from(caches).unwrap(),
                                );
                                warn!(%error, "Migration failed");
