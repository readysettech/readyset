...
use readyset_sql::{
    ast::{
        AddTablesStatement, AlterReadysetStatement, CacheInner, DropCacheStatement, Relation,
        ResnapshotTableStatement, SqlQuery,
    },
...
/// private `index` field. Our implementation is a little clunky because it replicates that
/// behaviorn but without being able to set `index` directly: instead, we just repeatedly set the
/// token to the previous one until it's back where we started.
fn parse_readyset_keywords(parser: &mut Parser, keywords: &[ReadysetKeyword]) -> bool {
    ...
    for &keyword in keywords {
        if !parse_readyset_keyword(parser, keyword) {
            // Reset position
            while parser.index() > start_index {
                parser.prev_token();
            }
            return false;
        }
    ...
    if parse_readyset_keyword(parser, ReadysetKeyword::READYSET) {
        if parse_readyset_keywords(
            ...
            Err(ReadysetParsingError::ReadysetParsingError(
                "expected RESNAPSHOT TABLE".into(),
            ))
        }
    } else {
...
fn parse_readyset_show(
    ...
    if parse_readyset_keyword(parser, ReadysetKeyword::READYSET) {
        if parser.parse_keyword(Keyword::VERSION) {
            ...
            dbg!(parser.peek_tokens::<5>());
            Err(ReadysetParsingError::ReadysetParsingError(
                "expected VERSION, STATUS, TABLES, ALL TABLES, or MIGRATION STATUS".into(),
            ))
        }
    } else {
...
fn parse_readyset_query(
    ...
    if parser.parse_keyword(Keyword::ALTER) {
        parse_readyset_alter(parser, dialect)
