                    ...
                    Expr::Row {..}
                        if exprs.iter().all(|e| {
                            match e {
                                Expr::Row { exprs, .. } => {
                                    exprs.iter().all(|e| {
                                    matches!(e, Expr::Literal(lit) if !matches!(lit, Literal::Placeholder(_)))
                                })},
                        ...
                        if self.autoparameterize_equals {
                            // FIXME(sqlparser): handle the special case mentioned in the comment just before this
                            if !exprs.is_empty() && matches!(exprs[0], Expr::Literal(_)) {
                                let _ = mem::replace(exprs, vec![Expr::Row {exprs: exprs.clone(), explicit: false}]);
                            };
                            ...
                            let exprs = mem::replace(
                                ...
                                exprs
                                    .iter()
                                    .map(|e| -> Result<Expr, ReadySetError> {match e {
                                        Expr::Row {exprs, ..} => {
                                            Ok(Expr::Row {
                            ...
                            let num_exprs: usize = exprs.iter().map(|e| match e {
                                Expr::Row { exprs, .. } => Ok(exprs.len()),
                                _ => unsupported!("Expected a ROW of placeholders"),
                            }).collect::<ReadySetResult<Vec<_>>>()?.into_iter().sum();
            ...
            match expression {
                ...
                Expr::In {
                    ...
                    match e {
                        ...
                        Expr::Row { exprs, ..} if exprs.iter().all(
