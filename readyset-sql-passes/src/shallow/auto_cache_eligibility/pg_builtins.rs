...
// Source: pg_catalog.pg_proc across PostgreSQL majors 13, 14, 15, 16, 17 (BUILTINS unioned, IMMUTABLE_BUILTINS intersected over the majors where each name exists).
// BUILTINS: every pg_catalog function (builtin-vs-UDF test).
// IMMUTABLE_BUILTINS: names whose every overload is provolatile='i' in every major where the name exists (default-deny cacheable base).
