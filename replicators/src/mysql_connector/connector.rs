...
impl MySqlBinlogConnector {
    ...
    fn map_partial_binlog_row_insert(
        ...
        for col in row_data.columns_ref() {
            if let Some(idx) = self.parse_column_index(&col.name_str()) {
    ...
    fn create_partial_update(
        ...
        let row_after = row.1.as_ref().ok_or_else(|| {
            mysql_async::Error::Other(Box::new(internal_err!("Missing data in DELETE_ROWS_EVENT")))
        })?;
    ...
    #[inline]
    fn parse_column_index(&self, col_name: &str) -> Option<usize> {
        col_name.get(1..).and_then(|s| s.parse::<usize>().ok())
    }
...
fn binlog_row_to_noria_row(
    ...
    binlog_row
        ...
        .iter()
        .map(|col| {
            let tme_idx = col.name_str().get(1..).unwrap().parse::<usize>().unwrap();
