...
use crate::readyset_adapter::{Connector, ReplicationAction};
                ...
                .map_err(|ev| format!("Unknown binlog event type {}", ev))?
            {
                ...
                EventType::WRITE_ROWS_EVENT => {
                    ...
                    for row in ev.rows(tme) {
                        ...
                        // represent that row
                        inserted_rows.push(readyset::TableOperation::Insert(binlog_row_to_noria_row(
                ...
                EventType::UPDATE_ROWS_EVENT => {
                    ...
                    for row in ev.rows(tme) {
                        // For each row in the event we produce a pair of ReadySet table operations to
                        ...
                        updated_rows.push(readyset::TableOperation::Insert(binlog_row_to_noria_row(
...
fn binlog_val_to_noria_val(
    ...
    match (col_kind, meta) {
        (ColumnType::MYSQL_TYPE_TIMESTAMP2, &[0]) => {
            ...
            Ok(time.try_into().unwrap()) // Can unwarp because we know maps derectly to ReadySet type
        }
