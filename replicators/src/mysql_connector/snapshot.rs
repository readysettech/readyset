...
impl MySqlReplicator {
    ...
    /// Spawns a new tokio task that replicates a given table to ReadySet, returning
    /// the join handle
    ...
    /// Copy all base tables into ReadySet
...
impl<'a> TableStream<'a> {
    ...
    pub async fn next<'b>(&'b mut self) -> ReadySetResult<Option<Vec<readyset_data::DataType>>> {
