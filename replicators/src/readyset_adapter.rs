...
impl NoriaAdapter {
    ...
    /// and an externally supplied ReadySet `ControllerHandle`.
    ...
    /// If ReadySet has no replication offset information, it will replicate the target database in its
    ...
    async fn start_inner_mysql(
        ...
        // TODO: it is possible that the binlog position from ReadySet is no longer
        ...
        // Only once binlog advanced to that point, can we send a ready signal to ReadySet.
    ...
    async fn start_inner_postgres(
        ...
        // Attempt to retreive the latest replication offset from ReadySet, if none is present
        ...
        if let Some(snapshot) = connector.snapshot_name.as_deref() {
            // If snapshot name exists, it means we need to make a snapshot to ReadySet
    ...
    /// Apply a DDL string to ReadySet with the current log position
    ...
    /// Send table actions to ReadySet tables, and update the binlog position for the table
    async fn handle_table_actions(
        ...
        let table_mutator = if let Some(table) = self.mutator_for_table(&table).await? {
            ...
            // found. Failing to execute an action for an existing table may very well get ReadySet
        ...
        // TODO: Make this operation atomic with the table
        // actions being pushed above.
        // TODO: We have to propagate txid to every table or
    ...
    /// Get a mutator for a ReadySet table from the cache if available, or fetch a new one
    /// from the controller and cache it. Returns None if the table doesn't exist in ReadySet.
