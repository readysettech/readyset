...
impl PostgresWalConnector {
    ...
    async fn send_standby_status_update(&mut self, cur_pos: Lsn) -> ReadySetResult<()> {
        ...
        lsn_to_ack.put_into(&mut b); // Acked
        lsn_to_ack.put_into(&mut b); // Flushed - this tells the server that it can remove prior WAL entries for this slot
        lsn_to_ack.put_into(&mut b); // Applied
        b.put_i64(now);
        b.put_u8(0);
        self.client
            .inner()
