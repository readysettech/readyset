...
//!   statement *in PL/PGSQL*, which we then convert to the ReadySet-native dialect by parsing it and
...
impl<'a> DdlEvent<'a> {
    ...
    /// Convert this [`DdlEvent`] into a SQL DDL statement that can be sent to ReadySet directly (using
    /// the ReadySet-native SQL dialect, not the postgresql dialect!)
