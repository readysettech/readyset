...
pub fn table_options(i: &[u8]) -> IResult<&[u8], ()> {
    map(separated_list(table_options_separator, create_option),|_| ())(i)
}

fn table_options_separator(i: &[u8]) -> IResult<&[u8], ()> {
    map(alt((multispace1, delimited(multispace0, tag(","), multispace0))),
        |_| ())(i)
}

fn create_option(i: &[u8]) -> IResult<&[u8], ()> {
    alt((create_option_type,
        ...
        create_option_key_block_size))(i)
}

fn create_option_type(i: &[u8]) -> IResult<&[u8], ()> {
    map(preceded(tag_no_case("type"),
...
fn create_option_pack_keys(i: &[u8]) -> IResult<&[u8], ()> {
    let (remaining_input, (_, _, _, _, _)) =
        tuple((tag_no_case("pack_keys"), multispace0, tag("="), multispace0,
            alt((tag("0"), tag("1")))))(i)?;
    Ok((remaining_input, ()))
}
...
fn create_option_engine(i: &[u8]) -> IResult<&[u8], ()> {
    let (remaining_input, (_, _, _, _, _)) =
        tuple((tag_no_case("engine"), multispace0, tag("="), multispace0,
               opt(alphanumeric1)))(i)?;
    Ok((remaining_input, ()))
}
...
fn create_option_auto_increment(i: &[u8]) -> IResult<&[u8], ()> {
    let (remaining_input, (_, _, _, _, _)) =
        tuple((tag_no_case("auto_increment"), multispace0, tag("="), multispace0,
               integer_literal))(i)?;
    Ok((remaining_input, ()))
}
...
fn create_option_default_charset(i: &[u8]) -> IResult<&[u8], ()> {
    let (remaining_input, (_, _, _, _, _)) =
        tuple((tag_no_case("default charset"), multispace0, tag("="), multispace0,
               alt((tag("utf8mb4"), tag("utf8"), tag("binary"), tag("big5"),
                    tag("ucs2"), tag("latin1")))))(i)?;
    Ok((remaining_input, ()))
}

fn create_option_collate(i: &[u8]) -> IResult<&[u8], ()> {
    let (remaining_input, (_, _, _, _, _)) =
        tuple((tag_no_case("collate"), multispace0, tag("="), multispace0,
...
fn create_option_comment(i: &[u8]) -> IResult<&[u8], ()> {
    let (remaining_input, (_, _, _, _, _)) =
        tuple((tag_no_case("comment"), multispace0, tag("="), multispace0,
               string_literal))(i)?;
    Ok((remaining_input, ()))
}
...
fn create_option_max_rows(i: &[u8]) -> IResult<&[u8], ()> {
    let (remaining_input, (_, _, _, _, _)) =
        tuple((tag_no_case("max_rows"), multispace0, opt(tag("=")), multispace0,
               integer_literal))(i)?;
    Ok((remaining_input, ()))
}
...
fn create_option_avg_row_length(i: &[u8]) -> IResult<&[u8], ()> {
    let (remaining_input, (_, _, _, _, _)) =
        tuple((tag_no_case("avg_row_length"), multispace0, opt(tag("=")), multispace0,
               integer_literal))(i)?;
    Ok((remaining_input, ()))
}
...
fn create_option_row_format(i: &[u8]) -> IResult<&[u8], ()> {
    let (remaining_input, (_, _, _, _, _)) =
        tuple((tag_no_case("row_format"), multispace0, opt(tag("=")), multispace0,
               alt((tag_no_case("DEFAULT"),
                    tag_no_case("DYNAMIC"),
                    tag_no_case("FIXED"),
                    tag_no_case("COMPRESSED"),
                    tag_no_case("REDUNDANT"),
                    tag_no_case("COMPACT")))))(i)?;
    Ok((remaining_input, ()))
}

fn create_option_key_block_size(i: &[u8]) -> IResult<&[u8], ()> {
    let (remaining_input, (_, _, _, _, _)) =
        tuple((tag_no_case("key_block_size"), multispace0, opt(tag("=")), multispace0,
               integer_literal))(i)?;
    Ok((remaining_input, ()))
}
