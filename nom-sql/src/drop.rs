...
pub fn drop_table(
    ...
    move |i| {
        ...
        let (i, temporary) = opt(preceded(whitespace1, tag_no_case("temporary")))(i)?;
        let (i, _) = whitespace1(i)?;
...
mod tests {
    ...
    fn drop_temporary_table_if_exists() {
        let res = test_parse!(
            drop_table(Dialect::MySQL),
            b"DROP TEMPORARY TABLE IF EXISTS users /* generated by server */;"
        );
        assert_eq!(res.tables, vec![Relation::from("users")]);
        assert!(res.if_exists);
        assert!(res.temporary);

        let res = test_parse!(
            drop_table(Dialect::MySQL),
            b"DROP TABLE IF EXISTS users /* generated by server */;"
        );
        assert_eq!(res.tables, vec![Relation::from("users")]);
        assert!(res.if_exists);
        assert!(!res.temporary);
